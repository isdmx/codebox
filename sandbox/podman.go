...
func (p *PodmanExecutor) Execute(ctx context.Context, req ExecuteRequest) (ExecuteResult, error) {
	...
	if len(req.WorkdirTar) > 0 {
		if err := p.extractTarToDir(req.WorkdirTar, workdirPath); err != nil {
	...
	codeFileName, err := p.getCodeFileName(req.Language)
	if err != nil {
		return ExecuteResult{}, fmt.Errorf("invalid language: %w", err)
	}

	codeFilePath := filepath.Join(workdirPath, codeFileName)
	if err := p.writeUserCode(req.Language, req.Code, codeFilePath); err != nil {
	...
	if len(artifactsTar) > p.config.MaxArtifactSizeMB*1024*1024 {
		return ExecuteResult{}, fmt.Errorf("artifacts size exceeds limit: %d bytes > %d bytes", 
			len(artifactsTar), p.config.MaxArtifactSizeMB*1024*1024)
	}
...
func (p *PodmanExecutor) getCodeFileName(language string) (string, error) {
	switch language {
	case "python":
		return "main.py", nil
	case "nodejs":
...
func (p *PodmanExecutor) writeUserCode(language, code, filePath string) error {
	...
	finalCode := p.applyHooks(language, code)

	return os.WriteFile(filePath, []byte(finalCode), 0644)
}

func (p *PodmanExecutor) applyHooks(language, code string) string {
...
func (p *PodmanExecutor) getRunCommand(language string) (string, error) {
	switch language {
	case "python":
...
func (p *PodmanExecutor) createTarFromDir(srcDir string) ([]byte, error) {
