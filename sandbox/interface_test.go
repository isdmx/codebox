...
func TestGetCodeFileName(t *testing.T) {
	tests := []struct {
		...
		{LanguagePython, FilenamePython, false},
		{LanguageNodeJS, FilenameNodeJS, false},
		{LanguageGo, FilenameGo, false},
		{LanguageCPP, FilenameCPP, false},
		{"invalid", "", true},
	}
...
func TestGetRunCommand(t *testing.T) {
	tests := []struct {
		...
		{LanguagePython, "python main.py", false},
		{LanguageNodeJS, "node index.js", false},
		{LanguageGo, "go build -o app main.go && ./app", false},
		{LanguageCPP, "g++ -std=c++17 -O2 -o app main.cpp && ./app", false},
		{"invalid", "", true},
	}
...
func TestApplyHooks(t *testing.T) {
	t.Run("PythonHooks", func(t *testing.T) {
		code := "print('hello')"
		result := ApplyHooks(LanguagePython, code, nil) // Second parameter ignored for backward compatibility
	...
	t.Run("NodeJSHooks", func(t *testing.T) {
		...
		result := ApplyHooks(LanguageNodeJS, code, nil) // Second parameter ignored for backward compatibility
		assert.Equal(t, code, result)
	})

	t.Run("GoNoHooks", func(t *testing.T) {
		code := "package main\nfunc main() { println(\"hello\") }"
		result := ApplyHooks(LanguageGo, code, nil) // Second parameter ignored for backward compatibility
		assert.Equal(t, code, result)
	})

	t.Run("CppNoHooks", func(t *testing.T) {
		code := "#include <iostream>\nint main() { std::cout << \"hello\"; }"
		result := ApplyHooks(LanguageCPP, code, nil) // Second parameter ignored for backward compatibility
		assert.Equal(t, code, result)
	})
...
func TestGetEnvironmentVariables(t *testing.T) {
	t.Run("PythonEnv", func(t *testing.T) {
		env, err := GetEnvironmentVariables(nil, LanguagePython) // First parameter ignored for backward compatibility
		require.NoError(t, err)
		assert.Equal(t, map[string]string{}, env) // Returns empty map for backward compatibility
	})

	t.Run("NodeJSEnv", func(t *testing.T) {
		env, err := GetEnvironmentVariables(nil, LanguageNodeJS) // First parameter ignored for backward compatibility
		require.NoError(t, err)
		assert.Equal(t, map[string]string{}, env)
	})

	t.Run("GoEnv", func(t *testing.T) {
		env, err := GetEnvironmentVariables(nil, LanguageGo) // First parameter ignored for backward compatibility
		require.NoError(t, err)
		assert.Equal(t, map[string]string{}, env)
	})

	t.Run("CppEnv", func(t *testing.T) {
		env, err := GetEnvironmentVariables(nil, LanguageCPP) // First parameter ignored for backward compatibility
		require.NoError(t, err)
		assert.Equal(t, map[string]string{}, env)
	})
